...
namespace PH
{
	...
	EntryAddr insert_entry_to_slot(NodeMeta* nodeMeta,unsigned char* src_addr, int value_size8) // need lock from outside
	{
		...
		if (entry_size > nodeMeta->max_empty)
	...
	void PH_Thread::split_listNode_group(ListNode *listNode,SkiplistNode *skiplistNode) // MAKE MANY BUGS
	{
		...
		while(list_nodeMeta)
		{
			...
			for (i=0;i<list_nodeMeta->el_cnt[0];i++) // find valid entries
			{
		...
		if (list_size_sum > NODE_SIZE)
		{
			...
			while(i < size-1) // size == 1 will go right...
			{
				...
				if (offset+entry_size > NODE_SIZE || j >= NODE_SLOT_MAX-1) // + JUMP
				{
	...
	EntryAddr PH_Thread::insert_to_cold(SkiplistNode* skiplistNode,unsigned char* src_addr, uint64_t key, int value_size, std::atomic<uint8_t>* &seg_lock, EntryAddr old_ea, bool large) // have skiplist lock // return old ea // need invalidation and kv unlock after this
	{
		...
		while(true) // always success unless re updated // split retry loop
		{
			...
			while (true)//list_nodeMeta) // try block group // group loop
			{
				...
				if (ENTRY_SIZE_WITHOUT_VALUE + value_size8 > list_nodeMeta->max_empty || ENTRY_SIZE_WITHOUT_VALUE + value_size8+list_nodeMeta->size_sum > NODE_SIZE-NODE_HEADER_SIZE) // check again after lock
				{
					at_unlock2(list_nodeMeta->rw_lock);
					continue;
				}
...
//				at_lock2(skiplist_node->key_list_lock);
				skiplist_node->remove_key_from_list(key); // here has evict lock and key lock and key list lock
...
#define INDEX
	...
	int PH_Query_Thread::insert_op(uint64_t key, int value_size, unsigned char* value)
	{
		...
		if (dtc)
		{
			...
			skiplistNode = get_skiplist_node_for_evict(key,warm_cache);
...
//					node = get_skiplist_node_for_key_list(key,warm_cache);
					...
					if (node->key_list_size >= WARM_KEY_LIST_MAX_TEMP) // need split
					{
						...
						if (may_split_warm_node(node,1/*3*/) == false) // has key list lock
						{
							//may impossible
...
//		at_unlock2(skiplistNode->key_list_lock);
	...
	void PH_Thread::flush_warm_node(SkiplistNode* node) // lock from outside // has key_list-lock!!
	{
...
//				at_lock2(node->key_list_lock); // conflict in hot to warm
	...
	{
		// data is batch // 
		return ((node->data_head-node->data_tail) >= WARM_GROUP_BATCH_CNT-1 -1 ); // if no space // batch >= 4 * 4 // -1 for spare
	}
	...
	void PH_Thread::warm_to_cold(SkiplistNode* node) // lock form outside
	{
		...
		while (i < end_slot)
		{
			...
			{
				//				nodeMeta->entryLoc[i].offset = 0; // don't do this // conflict with invalidation
				i++;
				continue;
			}
			...
			if (header.valid_bit)// && header.prev_loc != 3)// && is_checked(header) == false)
			{
					...
					//					if (node->list_head - node->list_tail >= NODE_SLOT_MAX) // temp
				{
					...
					if (need_hot_to_warm(node))
					{
						...
						soft_htw_cnt++;
						//							at_unlock2(node->insert_lock);
						if (may_split_warm_node(node,2) == false)
							at_unlock2(node->evict_lock);
					}
					else
						at_unlock2(node->insert_lock);
